...
public class BaseProxy {
    ...
    public static <T extends BaseRequestDTO, F> F doGetWithAutoDeserilize(String url, T dto) throws Exception {
        String response = doGet(url, dto);
        if (response == null) {
            log.error("DoGetWithAutoDeserilize方法=====>网络请求异常，请检查网络，请求url={},请求参数={}", url, dto.toString());
            throw new BizException("网络请求异常，请检查网络！");
        }

        BaseResponseDTO<F> result;
        try {
            result = mapper.readValue(response, new TypeReference<BaseResponseDTO<F>>() {
            });
        } catch (Exception exp) {
            log.error("mapper.readValue()=====>函数异常！,检测参数是否正确，异常信息e={}，请求url={},请求参数={}", exp.getMessage(), url, dto.toString());
            throw new Exception("mapper.readValue函数异常！");
        }
        if (result == null) {
    ...
    public static <T extends BaseRequestDTO, F> F doGetWithDeserilizeManual(String url, T obj, TypeReference<F> type) throws Exception {
        String response = doGet(url, obj);
        if (response == null) {
